...
decl_storage! {
	trait Store for Module<T: Config> as Staking {
		...
		pub ValidatorMinBond get(fn validator_min_bond) config(): BalanceOf<T>;
		...
		pub Validators get(fn validators):
			map hasher(twox_64_concat) T::AccountId => ValidatorPrefs<T::Hash>;
	...
	add_extra_genesis {
		config(stakers):
			Vec<(T::AccountId, T::AccountId, BalanceOf<T>, StakerStatus<T::Hash, T::AccountId>)>;
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		pub fn validate(origin, prefs: ValidatorPrefs<T::Hash>) {
			let controller = ensure_signed(origin)?;
			let ledger = Self::ledger(&controller).ok_or(Error::<T>::NotController)?;
			let stash = &ledger.stash;
			// Ensure validator has minimum bond
			ensure!(ledger.total >= <ValidatorMinBond<T>>::get(),
		...
		fn set_validator_min_bond(origin, #[compact] new: BalanceOf<T>) {
			ensure_root(origin)?;
			<ValidatorMinBond<T>>::put(new);
		}
